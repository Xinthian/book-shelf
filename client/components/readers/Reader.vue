<template>
  <div v-if="show" id="reader" :data-theme="ereaderTheme" class="group absolute top-0 left-0 w-full z-60 data-[theme=dark]:bg-primary data-[theme=dark]:text-white data-[theme=amoled]:bg-black data-[theme=amoled]:text-white data-[theme=light]:bg-white data-[theme=light]:text-black data-[theme=sepia]:bg-[rgb(244,236,216)] data-[theme=sepia]:text-[#5b4636]" :class="{ 'reader-player-open': !!streamLibraryItem }">
    ...
    <div
      v-if="isEpub"
      class="w-96 h-full max-h-full absolute top-0 left-0 shadow-xl transition-transform z-30 group-data-[theme=dark]:bg-primary group-data-[theme=dark]:text-white group-data-[theme=amoled]:bg-black group-data-[theme=amoled]:text-white group-data-[theme=light]:bg-white group-data-[theme=light]:text-black group-data-[theme=sepia]:bg-[rgb(244,236,216)] group-data-[theme=sepia]:text-[#5b4636]"
    ...
    <modals-modal v-model="showSettings" name="ereader-settings-modal" :width="620" :height="'unset'" :processing="false">
      <template #outer>
      ...
      <div class="px-2 py-4 md:p-8 w-full text-base rounded-lg shadow-lg border border-black-300 relative overflow-x-hidden overflow-y-auto" :class="ereaderTheme === 'amoled' ? 'bg-black' : 'bg-bg'" style="max-height: 80vh">
        <div class="flex flex-col sm:flex-row sm:items-center mb-4">
          <div class="w-full sm:w-40 shrink-0 mb-2 sm:mb-0">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center mb-4">
          <div class="w-full sm:w-40 shrink-0 mb-2 sm:mb-0">
            <p class="text-lg">Fast font:</p>
          </div>
          <ui-toggle-btns v-model="ereaderSettings.font" :items="themeItems.fastFont" @input="settingsUpdated" />
        </div>
...
export default {
  ...
  computed: {
    ...
    isDarkTheme() {
      return this.ereaderSettings.theme === 'dark' || this.ereaderSettings.theme === 'amoled'
    }
  },
  methods: {
    goToChapter(uri) {
